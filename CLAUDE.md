# Clasificador y Enrutador de Correos — areamarketing@inelinc.com
...
1. Listar los correos de la bandeja de entrada de `areamarketing@inelinc.com` recibidos en las **últimas 24 horas** que **NO** estén ya en la carpeta `Procesados`.
...
4. **Idempotencia**: nunca proceses dos veces el mismo correo. La carpeta `Procesados` es la fuente de verdad — si un correo ya está ahí, ignóralo. Si la carpeta `Procesados` no existe dentro de la bandeja de `areamarketing@inelinc.com`, créala antes de empezar.
...
|---|---|---|
| Listar/leer correos nuevos de `areamarketing@inelinc.com` | **Outlook MCP (Composio)** | Filtrar por carpeta inbox, excluyendo los que ya están en `Procesados` |
